import { garagePage, racingBlock } from './pageElements';
import { startEngine, stopEngine, basis } from './API';
import { selectCar } from './updateCar';
import { removeCarFunc, removeWinnerFunc } from './deleteCar';

function addCars(carName, carColor, carID) {
  const racingCarBlock = document.createElement('div');
  ...
  racingCarBlock.innerHTML = `
    ...
    <img src="./src/images/stop.png" width="30" class="stop" id="stop-${carID}"/>
    <img src="./src/images/play.png" width="30" class="play" id="play-${carID}"/>
    </div>
    <span class="car-status" id='${carID}-status'></span>
     ...
     <div class="racing-flag"><img src="./src/images/flag.png" alt="racing-flag"></div></div>
     <div class="racing-track"><img src="./src/images/track.png" alt="racing-track"></img></div>`;
  ...
  selectBtn.addEventListener('click', () => removeCarFunc(carID), removeWinnerFunc(carID));
...
garagePage.insertAdjacentElement('beforeend', racingBlock);

const renderCar = async () => {
  const cars = await basis();
  cars.forEach((el) => addCars(el.name, el.color, el.id));
  return cars;
};

export { addCars, renderCar };
